...
Singleton {
    ...
    property string uptime: "0d 0h 0m" // placeholder, indicative
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (root._timertick % 30 === 0) {
                pollDiskUsage.running = true;
                pollLocalIp.running = true;
                pollGlobalIp.running = true;
                root._timertick = 0;
            }
        }
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: root.localIp = text.trim().split(" ")[0]
        }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: root.globalIp = text.trim()
        }
    ...
    Process {
        id: pollOSName
        command: ["sh", "-c", "source /etc/os-release && echo $PRETTY_NAME"]

        stdout: StdioCollector {
            onStreamFinished: {
                root.osName = text.trim().split("|")[0];
            }
        }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: root.procCount = parseInt(text.trim())
        }
    }
    ...
    Process {
        ...
        command: ["uname"]
        stdout: StdioCollector {
            onStreamFinished: root.kernelName = text.trim()
        }
